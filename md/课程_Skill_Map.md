...
> 更新：2026-09，整合 cangjie-skill-main 逻辑底层 + S01-S11 案例 + B01-B20 章节适配
...
AIGC 版 Skill Map 覆盖 9 个视觉生成 Skill，解决"分析结论→视觉作品"。课程版做转换——**以 cangjie-skill-main 蒸馏的 15 个逻辑 skill 为底层，supergrill 为方法论内核**，把 Agent/Skill 工程（S01-S11，其中 S09/S11 兼具可视化）、数据可视化（B02 统计图表 + B20 C4 架构图 + S09 动态架构图）、交付治理（B05 TDD / B16 multi-agent-review）串成一条**逻辑闭环**：每个 Skill 对应课程具体章节，每个案例可追溯到逻辑方法论。
...
└───────────────────────────┬──────────────────────────────────────┘
                            ▼
┌──────────────────────────────────────────────────────────────────┐
│  A. Agent/Skill 工程层  ← S01-S11（分诊/体检/简报/机会图/         │
│     海报/PPTX/PixiJS/3D/动态图/UI 工作台/GeoJSON 地图）           │
...
### A. Agent/Skill 工程层（S01-S11 · 对应 03-Agent与Skill工程.md）
...
| 5 | poster-recipe | S05 海报 | LiamGvchi/gc-minimal-zine-poster、awesome-gpt-image-2、visual-memory-translator（视觉与 Prompt 参考） | 三套方向+本地 SVG；A/B/C/D Prompt-Skill 同题对比 |
| 6 | slide-plan + HTML 演示对比 | S06 PPTX/HTML 竞技场 | 课程自有；zarazhangrui/frontend-slides；chuspeeism/dashi-ppt-skill | 同一大纲→可编辑 PPTX、零依赖 HTML、每页 3+1 模板探索与浏览器验收 |
...
| 10 | ui-workbench | S10 Prompt/Skill/UI Kit 分工 | 课程自有；Tabler 仅为 UI Kit | B10 数据→工作流、状态门、C/D 同合同页面与浏览器验收 |
| 11 | three-scope-map | S11 真实 GeoJSON 地图 | songsummer920-dazzle/three-scope-map-skill（GPL-3.0） | Earth→中国→浙江下钻；与 CSS 伪 3D、手写 Three.js 同题实测 |
...
> 以上 30 个 Skill 分属三层，但清单只是目录——真正的价值在**逻辑闭环**：每个工程 Skill 执行时，逻辑底层 skill 在方法论上把关，supergrill 在选型前拷问，治理层在产出后验收。以下 5 条主线展示闭环怎么跑。
...
> 5 条主线跑通后，每个 Skill 的来源、许可、star 都需要核实——开源 Skill 克隆到本地不等于拥有，课程保留的是实际采用的触发条件、输入输出和测试。以下来源表供核实。
...
| poster-recipe | 课程自有；外部 Prompt Skill 固定版本位于 `skills/` | — | S05 海报配方与 Prompt-Skill 对比 |
| slide-plan | 课程自有 | — | S06 可编辑 PPTX 与轻量 HTML 适配器 |
| frontend-slides | zarazhangrui/frontend-slides（MIT，固定 commit `9906a34d...`） | — | S06 单文件 HTML 规程，桌面/手机浏览器实测 |
| dashi-ppt | chuspeeism/dashi-ppt-skill（AGPL-3.0，固定 commit `21dc7e5f...`） | — | S06 每页 3 模板 + 1 定制方案；记录单位、移动端和依赖成本 |
| ui-workbench | 课程自有 | — | S10 后台流程、状态与可访问性验收；与 UI Kit 分责 |
| Tabler | `@tabler/core@1.4.0`（MIT，UI Kit，不是 Skill） | — | S10 按钮、徽标、表格和边框的一致性对照 |
| three-scope-map | songsummer920-dazzle/three-scope-map-skill（GPL-3.0，固定 commit `605867c0...`） | — | S11 真实 GeoJSON、Earth 入口、材质、hover 和下钻；移动端需专项适配 |
...
4. **最后整合**：把 5 条逻辑闭环主线的"结果图待补充"项逐一补齐，链接回本文档
