...
![AI 工作循环](../assets/diagrams/loop.png)
...
结果见 [L01 运行记录](../assets/loop-results/loop-run-L01.json) 和 [经营简报](../assets/L01/business-brief.md)。四项条件都通过后停止。
...
[L02 运行记录](../assets/loop-results/loop-run-L02.json) 停在 `visual_choice_required`，由负责作品的人选择最终方向。
...
源数据的 `maintenance_action_allowed` 全部为 `False`，资料也没有真实维修结果。因此系统只生成 [现场检查申请](../assets/L03/approval-packet.json)，不会写故障根因，更不会停机、维修或调用设备工具。
...
[L03 运行记录](../assets/loop-results/loop-run-L03.json) 停在 `permission_required`：材料够整理申请，还不能替人做高影响决定。
