...
![Skill Registry 生命周期](../assets/diagrams/fig_skill_lifecycle.svg)
...
> skill-scanner 扫的是反例（提示注入、恶意代码）而非正例——这正是**证伪思维**：不是问"这个 Skill 看起来正常吗"，而是问"有没有一条反例让它通不过"。就像安全扫描不在证明系统安全，而在找已知漏洞——找反例永远比找正例更有效。
...
> 六槽结构（触发条件/输入/澄清问题/PRD/验收/复用范围）确保每个 Skill 的概念含义在团队中保持一致——这正是**同一律**：每个槽位的含义不能偷换。"触发条件"不能写成"使用场景"，"验收标准"不能写成"期望效果"。六槽是同一律的工程化落地——用结构约束保证每个概念在团队中只有一个权威表示。
...
![Skill 分发：Registry → 多 Agent](../assets/diagrams/fig_skill_distribution.svg)

开源 Skill 只作为结构参考；课程保留实际采用的触发条件、输入输出、许可和测试，不维护容易过时的工具排行榜。

---

### 补充：个人 Skill 的分层迭代与优化

团队治理解决"共享与安全"，个人迭代解决"越用越乱"。Skill 用久了，主文件容易臃肿——图片出错加一条规则、流程出问题加一个补丁，最终核心流程被淹没。黄金原则是：SKILL.md 只留核心流程和不变规则，细节拆到 references/，模板素材拆到 assets/。这与 §6.6 的 `skill_lint` 检查的六槽结构一致——结构完整不是靠人记，而是靠门禁守。

当 Skill 需要优化时，可以用下面的指令模板让 Agent 帮你分层整理：

```text
帮我优化这个 Skill，具体要求：

## 需要改进的问题
【如：触发不准、某步骤频繁出错、输出格式不稳定、内容冗余等】

## 改进方向
【如：优化触发条件、增加边界校验、补充实操案例、精简主文件等】

## 重要规则
1. 新增内容精准分类：
   - 核心不变规则 → 保留 SKILL.md
   - 案例、细节规则、历史记录 → 归入 references/
   - 模板、素材、配色 → 归入 assets/
...
3. 严控主文件简洁度：
   - SKILL.md 控制在 200 行以内
   - 所有复杂解释、实操案例，统一拆分到子文档
