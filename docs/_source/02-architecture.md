...
要诚实的是：SDD 不是银弹。代码生成本身**非确定**，规格也可能漂移、也可能滑向「重型瀑布」。所以它必须配一套**高确定性的 CI 门禁**兜底——这正好是本书 §6 的三绿门禁存在的理由。**方法论（分步）+ 门禁（把关）** 两条腿，才走得动一个中大型系统。（来源：GitHub Spec Kit；Thoughtworks 2025 定名。）
```
