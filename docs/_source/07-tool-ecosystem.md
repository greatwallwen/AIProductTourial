...
> **工具信息最后核实：2026-07。** 上面的具体工具名、命令、星数都会变（AI 工具生态迭代极快）——但那条**流水线（治理→规格→纪律→自动化）是模式，不随工具过时**。记模式，工具随时可换；详见下方 B.3 的开源资源索引与「过时即风险」。

#### B.1a 国产落地：一个当下就能装、能跑的组合拳实例——CodeBuddy
...
> **前置**：在系统终端运行，需 Node ≥ 18（`npx` 随 Node 自带）；装好的技能由 Claude Code / Codex 等 Agent 读取使用。

```bash
npx skills@latest add mattpocock/skills   # 选装（非全装），经 vercel-labs/skills CLI 层面支持多 Agent（README 只点名 Claude Code/Codex）
```

该仓库自身就是「过时即风险」的活例：早期 ~18 个技能 → v1.0 改名 diagnose、删 zoom-out/caveman → 现 main 已 36 个 SKILL.md——**命令与清单会变，「教员式」这根轴不变**（最后核实 2026-07）。
...
> **前置**：需先自部署 **Nacos ≥ 3.2**（Docker 或二进制，见官方仓库 alibaba/nacos）并启用 AI 注册中心模块，`nacos-cli` 另行安装；没有部署就跑下列命令只会连接失败——个人/小团队可先用 §6.5 的 git 平替。
...
| 教员式·拷问/教学 | mattpocock/skills（MIT） | 反向面试 grill/tdd/triage 等~36 个小技能；与 gstack /office-hours 同属「教员侧」 |
