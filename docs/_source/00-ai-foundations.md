...
**③ 因果 ≠ 相关。** 休谟说我们从没观察到「必然的因果」，只观察到「恒常联结」——A 之后总跟着 B。现代说法（Judea Pearl《为什么》的「因果之梯」）：机器学习学的是**相关**，不是因果，站在阶梯最低一层。这正是本书数据篇反复警惕的「把噪声/相关当信号」。
...
> 🟢 **必读** ｜ ★☆☆ ｜ 关键词：**概念 ↔ 章节 ↔ 案例**（本章不是孤岛，每个概念后面都会用到）

本章的概念不是背完就完——它们贯穿全书。遇到时用这张表随时往后跳，也能理解「为什么要先学这个」：

| §1 概念 | 后文在哪儿落地（章节 / 案例） |
|---|---|
| Token / 上下文窗口 | §2 Loop 的成本-延迟权衡；案例 44 RAG「只喂高相关片段」正是为省 Token |
| RAG / Embedding / 向量 | 案例 44 语义检索、案例 45 关系库底座、案例 49 RAG 评测台；§3.3 用它做检索上下文 |
| Agent / ReAct（§1.6） | §2 三层 Loop 的「执行器」；案例 48 CI 失败分诊 Loop |
| eval / 评测 | §2「传感器就是设计本身」、§6 门禁三绿；案例 49 评测台、案例 50 交付门禁 |
| 幻觉 / hallucination | 案例 44 用检索压幻觉；§6 高影响域保留人工复核 |
