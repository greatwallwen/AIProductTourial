...
![Tokenizer 编码：文字→Token→Token ID](assets/course/image2/ai-tokenizer-flow.png)
...
![RAG 回答流程：Embedding→向量库→召回→重排→上下文→大模型](assets/course/image2/rag-retrieval-flow.png)
...
![System Prompt 定边界 · User Prompt 定任务](assets/course/image2/system-user-prompt.png)
...
问题又来了：不同 AI 应用怎样发现外部能力、交换参数和结果？MCP（模型上下文协议）为这段**上下文交换**定义了 JSON-RPC 客户端/服务器协议。连接先初始化并协商版本与能力；服务器可暴露工具、资源和提示词，客户端可按声明支持 roots、sampling、elicitation 等能力；传输可用本地 stdio 或远程 Streamable HTTP。

Type-C 只能帮助记住「有统一连接面」，不能替代协议边界：MCP 不会自动统一业务语义，不会替你完成授权，也不保证某个服务器安全。客户端只能调用协商成功且用户允许的能力。附录B 的最小实验会真实跑一次 `initialize → list → call`，再尝试一个未发现工具，确认它被拒绝。
```


![工具调用：用户、宿主、模型与工具的职责边界](assets/course/image2/tool-call-contract.png)
...
可 Agent 再聪明，面对团队特有流程还是会抓瞎。Agent Skill 把触发边界、工作流、验证方式和按需资源放进一个可发现的目录包：发现阶段只看元数据，激活时读 `SKILL.md`，执行中才按需加载参考文件。它不是“把所有知识塞进一条长 prompt”。本教程的六槽卡是知识卡格式；真正的 Skill 包与渐进披露实验在 §6。
```

![Agent 的 ReAct 闭环：判断→调用工具→观察→再判断](assets/course/image2/react-loop.png)
