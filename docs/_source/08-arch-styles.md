...
```备注
这张表就是本章的用法：忘掉某个中间件的名字没关系，记住**模式约束的是什么**——然后在你的 AI 系统里找同构物。传统书教你 Memcached/Redis/Nginx 的配置，本书教你的是：下次见到「上下文缓存命中率」，你能认出这就是 5.1 节的缓存问题换了host。
```
