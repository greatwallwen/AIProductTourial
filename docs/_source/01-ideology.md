...
传感器分两类：**确定性校验**管「有没有」，**概率性评测（evals）**管「好不好」（金标集+裁判打分）。最大的坑：**裁判必须真调被测系统**——案例 49 的门第一版只量语料覆盖，把 `search()` 改坏照样绿；第二版真跑检索数 hit@3 才现形：覆盖 91.7%、命中仅 25%。这个差距才是能驱动 Loop 的误差信号。
...
![评测鸿沟：语料有≠检索中（案例49实测）](outputs/product_case_library/svg/fig_eval_gap.svg)
...
一个独立收敛的旁证：mattpocock/skills（16 万 star 量级的外部技能库，星数仅弱信号）在 README「Why These Skills Exist」里用**同一张失败清单**组织全部技能——①Agent 没按你想的做（本书=意图债务→skill requirement-grill）；②话太啰嗦没共享语言（→§8.1 通用语言）；③代码跑不通（→§2.3 传感器→skill test-first）；④代码库熵增（→critic big-file+案46 适应度函数）。两套体系独立长出同一张清单——这比任何互相引用都更能说明这四个坑是真的。
```
