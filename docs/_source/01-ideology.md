...
传感器分两类，别混用：**确定性校验**管「有没有」（文件在不在、字段对不对、值域越不越界——一是一二是二）；**概率性评测（evals）**管「好不好」（检索答得准不准、生成对不对——要金标集+裁判打分）。有个必须记住的坑：**裁判必须真调被测系统**。本书案例 49 的评测门第一版只量「语料里有没有答案」——把检索函数改坏，门照样绿；第二版才让裁判真跑 `search()` 数 hit@3，一跑就现形：语料覆盖 91.7%，检索命中只有 25%。差距就是改进空间，这才是能驱动 Loop 的误差信号。
```
