...
传感器分两类：**确定性校验**管「有没有」，**概率性评测（evals）**管「好不好」（金标集+裁判打分）。案例 07 的评测门在本书里连演了三幕，每一幕都在还同一个债。第一幕**假绿**：裁判只量语料静态覆盖，报 91.7%，好看——可把 `search()` 改坏，门照样绿，它根本没碰过被测系统。第二幕**真调**：裁判改成真跑检索数 hit@3，立刻现形，25%。第三幕**查装载**：追问为什么这么低，才发现向量库默认只装载 80 篇，而语料实有 194 篇——被测系统连题库的一半都没见过；修复装载后实测 41.7%。余下的未命中要诚实注明：多数属金标自身缺陷（12 题里 5 题的期望文档在语料文件名中不存在），这套金标封顶 58.3%。三幕同一句话：**裁判必须真调被测系统，还得连装载一起验**——这样的误差信号才配驱动 Loop。
...
### 2.8c Loop 2：把“继续”也做成受控决策
> 🟢 **必读·动手** ｜ ★★☆ ｜ 关键词：**最大轮次** · **失败指纹** · **暂停/恢复** · **trace**

```备注
真正的 Loop 不是一条 `while(true)`。每轮至少记录目标、输入状态、动作、工具结果、评估、耗时和证据引用；编排器再根据结构化结果决定完成、继续、暂停审批或升级。最大轮次只防无限循环，失败指纹负责识别「同一个错重试两次」，预算负责限制成本，回归探针负责发现修一处坏一处。

OpenAI Agents SDK 的运行模型提供一个可对照的工程样本：模型输出最终结果时结束；工具调用执行后回到 Loop；handoff 切换 Agent；`max_turns` 给循环硬上限；Human-in-the-loop 可以暂停并从 `RunState` 恢复；tracing 记录模型、工具、handoff 与 guardrail 事件。多 Agent 文档也给出 evaluator 循环和并行专家两种模式。这只是其中一种实现，但它说明：**循环、审批、评估和追踪必须是显式状态，不该只存在聊天记忆里。**
```
...
第一条必须两轮收敛并输出 `stopReason: all-green`；第二条即使没成功，也必须因失败指纹重复而输出 `stopReason: repeated-failure` 和完整 `escalationPacket`。第二条若继续烧到五轮，说明停机规则只是文档，没有进入编排器。

对应课程活动 03/04，评分看 trace、哈希和升级包。完整实验见 `course/labs/01-evidence-loop.md`。实现概念可对照 [Agents SDK 运行循环](https://openai.github.io/openai-agents-python/running_agents/)、[Human-in-the-loop](https://openai.github.io/openai-agents-python/human_in_the_loop/)、[Tracing](https://openai.github.io/openai-agents-python/tracing/) 与 [多 Agent 模式](https://openai.github.io/openai-agents-python/multi_agent/)。
...
4. **实操**：运行 §2.8c 两条轨迹，比较 `stopReason`、`failureFingerprint`、`evidenceRefs` 和 `escalationPacket`，说明哪一项仅靠最大轮次无法替代。
