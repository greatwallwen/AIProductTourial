...
```备注
你有没有接手过一个三千行的文件？改一个小功能，得先滚半天鼠标找到位置，还生怕碰坏别处——这就是「可读性债」，迟早要还。所以本教程给自己立了条硬规矩：**单文件 < 800 行，超了就必须拆**，而且用 `verify` 自动卡住，谁都别想偷懒（本仓库所有源文件都真过了这条线，不是嘴上说说——下图是构建期实扫的 Top9 行数证据）。
```
...
> 🟢 **必读** ｜ ★★☆ ｜ 关键词：**YAGNI** · **复用优先** · **平台原生**（呼应 §3.6 奥卡姆）
