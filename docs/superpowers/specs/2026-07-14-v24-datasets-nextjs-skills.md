...
- ✅ **P3 接线 8/10 全绿**（续）：
  - `b70f4ea` 04/07 **CMRC→webMedQA 医疗垂直 KB**（big-bang：848 语料删→600 医疗语料，eval 重基线 hit@1=98.3%，fetch-datasets 语料块重写+REAL_SOURCES 补齐全部真集防 MANIFEST 重生成丢行，删 cmrc2018_dev.json，全链传播查询/§2.3/§9.5/def/守卫）
  - `71799eb` 02 新客vs复借 · `9c6c19f` 01/03 细分显式化（v23 已含 base-rate/真RFM）
  - 反思修 3 真 bug（若依边 28→18、死 three、门禁 hit@3→hit@1 静默 null）
- ⏳ **P3 剩 1**：**案例10 运营商客户投诉升级（新案·v22 规模）**——数据受限（工信部聚合仅 2 季度已核实：2021Q3=36199件/57.7·24.8·17.5%、2020Q2=37543件；WebFetch 地域封锁拿不到更多）→ 真值锚（转录真实聚合）+ 合成明细工单（标红线）；需新案 def/服务/屏/守卫/截图/案例数 9→10。
