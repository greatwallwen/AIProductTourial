...
**PASS** — 指标链 4 项均为回到 `rules/ + docs/_source + case_definitions + verify + 架构图（本仓库自身·dogfood）` 的实际计算值（真实数据）；字段/异常/Skill 齐备；可运行原型见 `#/case/08`（设计 cyan-matrix），截图 `assets/screenshots/premium_case_08_sdd_system_build_desktop.png`。
