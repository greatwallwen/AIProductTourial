 ...
 *  主指标 hit@1——期望文档模式必须是 store.ts 真实 search() 重排的第 1 名（v22：语料换中文 CMRC2018，hit@3 近乎全过、收紧到 hit@1 才有区分度）；次指标 coverage（语料覆盖≥3 篇）。
...
const CORPUS = join(ROOT, 'dataset', 'rag', 'corpus'); // v22：中文语料 CMRC2018（store.ts 二元组分词）；金标 code/data/eval_gold.json ← dataset/rag/gold.json
...
const results = gold.map(({ q, kw, docPattern }) => {
  ...
  const r = vs.search(q, 3, 10); // 被测系统真实调用：召回10→重排3（top3 留作错误分析展示）
  const top3 = (r.reranked || []).map((h) => h.id || h.file || h.title || '');
  const hit = top3.length ? re.test(String(top3[0])) : false; // hit@1：只认重排第 1（v22 收紧——hit@3 在 CMRC 上近乎全过，测不出区分度）
...
if (process.argv.includes('--update')) { writeFileSync(bp, JSON.stringify({ score, judge: 'hit@1(rerank top-1)', gold: gold.length, coverage: covOk }, null, 2) + '\n'); console.log(`基线已更新 → hit@1=${score}%`); process.exit(0); }
