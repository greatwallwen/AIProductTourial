...
function sddPipeline(t) {
  ...
  const nodes = steps.map(([label, sub], i) => ({
    id: 's' + i, x: x0 + i * (w + gap), y, w, h, label, sub,
...
function c4Context(t) {
  return diagram({
    ...
    nodes: [
      { id: 'user', x: 60, y: 170, w: 180, h: 76, tag: '人', color: t.accent2, label: '学习者', sub: '研发 / 产品 / 项目' },
      { id: 'sys', x: 340, y: 150, w: 220, h: 110, color: t.accent, label: 'AI 一体化知识库平台', sub: '真数据·可运行·自校验' },
      { id: 'data', x: 660, y: 70, w: 190, h: 76, tag: '外部', dashed: true, color: t.ink2, label: '公开数据集', sub: 'UCI / CMS / US DOT' },
      { id: 'ai', x: 660, y: 190, w: 190, h: 76, tag: '外部', dashed: true, color: t.ink2, label: 'AI 编程工具', sub: '任一 Agent 工具' },
      { id: 'br', x: 660, y: 300, w: 190, h: 64, tag: '外部', dashed: true, color: t.ink2, label: '浏览器', sub: '大屏 / 实验室' },
    ],
    edges: [
...
function c4Container(t) {
  return diagram({
    ...
    nodes: [
      { id: 'user', x: 800, y: 96, w: 150, h: 60, tag: '人', color: t.accent2, label: '学习者', sub: '浏览器' },
      { id: 'ds', x: 792, y: 452, w: 170, h: 70, tag: '外部', dashed: true, color: t.ink2, label: '公开数据集', sub: 'UCI / CMS / DOT' },
      { id: 'web', x: 96, y: 106, w: 300, h: 66, tag: '容器', label: 'web · React SPA', sub: '#/case/NN 大屏 + 实验室' },
      { id: 'verify', x: 430, y: 106, w: 296, h: 66, tag: '容器', color: t.warn, label: 'verify · 校验器(checker)', sub: '数百项门禁 · 只读' },
      { id: 'api', x: 96, y: 232, w: 300, h: 66, tag: '容器', label: 'api · Fastify routes', sub: '/api/* 契约 · OpenAPI 自动生成' },
      { id: 'svc', x: 96, y: 358, w: 300, h: 66, tag: '容器', label: 'services · 业务服务', sub: 'cases / rfm / hospital / dispatch…' },
      { id: 'db', x: 430, y: 358, w: 140, h: 66, tag: '存储', color: t.ok, label: 'db · node:sqlite', sub: '真 SQL 聚合' },
      { id: 'vec', x: 588, y: 358, w: 138, h: 66, tag: '存储', color: t.ok, label: 'vector · TF-IDF', sub: 'RAG 召回+重排' },
    ],
    edges: [
...
function c4Component(t) {
  return diagram({
    ...
    nodes: [
      { id: 'routes', x: 70, y: 110, w: 250, h: 70, tag: '组件', label: 'routes/api.ts', sub: '只收发 HTTP · 不写业务' },
      { id: 'cases', x: 360, y: 110, w: 250, h: 70, tag: '组件', label: 'services/cases.ts', sub: 'caseData · 预计算读取' },
      { id: 'biz', x: 650, y: 110, w: 220, h: 70, tag: '组件', label: '业务服务', sub: 'rfm / hospital / dispatch / retail' },
      { id: 'store', x: 360, y: 250, w: 250, h: 70, tag: '组件', label: 'vector/store.ts', sub: '两阶段召回→重排' },
      { id: 'openapi', x: 650, y: 250, w: 220, h: 70, tag: '组件', color: t.warn, label: 'openapi 契约', sub: '/api/openapi.json 自动生成' },
    ],
    edges: [
...
function dddContext(t) {
  return diagram({
    ...
    nodes: [
      { id: 'ingest', x: 60, y: 100, w: 220, h: 84, color: t.accent2, label: '采集接入 上下文', sub: 'CSV / 快照 → 归一化' },
      { id: 'govern', x: 380, y: 100, w: 220, h: 84, color: t.accent2, label: '数据治理 上下文', sub: 'metricSpec · 异常列 · 真源' },
      { id: 'insight', x: 700, y: 100, w: 220, h: 84, color: t.accent, label: '指标洞察 上下文', sub: 'KPI 真算 · 队列 · 图表' },
      { id: 'rag', x: 60, y: 300, w: 220, h: 84, color: t.accent, label: '检索(RAG) 上下文', sub: '向量库 · 召回 · 重排' },
      { id: 'decide', x: 380, y: 300, w: 220, h: 84, color: t.ok, label: '经营决策 上下文', sub: '方案 · 责任 · 验收' },
      { id: 'govn', x: 700, y: 300, w: 220, h: 84, color: t.warn, label: '交付治理 上下文', sub: 'L0-L3 · 门禁 · 风险登记' },
    ],
    edges: [
...
function deployment(t) {
  return diagram({
    ...
    nodes: [
      { id: 'fastify', x: 100, y: 130, w: 420, h: 60, tag: '进程', label: 'Fastify 服务（:5200）', sub: '托管 web/dist + 全部 /api' },
      { id: 'sqlite', x: 100, y: 230, w: 200, h: 60, tag: '文件', color: t.ok, label: 'node:sqlite', sub: '进程内 · 零外部 DB' },
      { id: 'files', x: 330, y: 230, w: 190, h: 60, tag: '文件', color: t.ok, label: 'dataset/ 快照', sub: '构建期装载' },
      { id: 'browser', x: 640, y: 200, w: 190, h: 66, tag: '客户端', color: t.accent2, label: '浏览器', sub: '#/case/NN' },
    ],
    edges: [
