...
const objects: CaseProjection[] = rows.map((payload) => ({
  caseId: "11",
  objectId: `11-${payload.evaluation_id}-MODEL-ADMISSION-001`,
  state: "待会签",
  version: 0,
  payload,
...
describe("ModelAdmissionWorkbench", () => {
  ...
  it("selects a metric within the same candidate and policy and opens the local report", () => {
    const duplicate: CaseProjection = {
      ...objects[7],
      objectId: "11-EVAL-11-008-MODEL-ADMISSION-OTHER",
    ...
    expect(onSelect).toHaveBeenCalledWith("11-EVAL-11-008-MODEL-ADMISSION-001");
