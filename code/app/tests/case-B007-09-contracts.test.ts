...
function projection(
  ...
  return {
    caseId,
    objectId: caseId === "B007"
      ? "B007-CN-FC-COURSE-01-2026-07-14"
      : caseId === "B009"
        ? "B009-METROPT-20200418-GAP-01"
...
describe("case 07 window decisions", () => {
  ...
  it("binds evidence and observability requests to the selected review window", () => {
    expect(() => validateDomainCommand({
      ...
      evidenceIds,
      data: task,
      current: projection("B007", "待评审", payload),
    })).not.toThrow();
    ...
    expect(() => validateDomainCommand({
      ...
      current: projection("B007", "架构评审中", payload, task),
    })).not.toThrow();
  });
});
...
describe("case 08 three-role field evidence", () => {
  ...
  it("separates dispatch, field return and supervisor acceptance", () => {
    expect(() => validateDomainCommand({
      caseId: "B008", command: "dispatch_field_check", actorRole: "dispatcher", actorId: "case08-dispatcher",
      idempotencyKey: "case08:dispatch:1", evidenceIds: ["CN-AQ-02-038"], data: { dispatch },
      current: projection("B008", "待研判", payload),
    })).not.toThrow();

    expect(() => validateDomainCommand({
      caseId: "B008", command: "submit_field_return", actorRole: "field_operator", actorId: "AQ-FIELD-02",
      idempotencyKey: "case08:return:1", evidenceIds: ["PHOTO-CN-AQ-02-038"], data: { fieldReturn },
      current: projection("B008", "现场取证中", payload, { dispatch }),
    })).not.toThrow();

    const returned = projection("B008", "待主管采信", payload, { dispatch, fieldReturn });
    const validation = {
      issueResolved: true,
    ...
    expect(() => validateDomainCommand({
      caseId: "B008", command: "confirm_event", actorRole: "supervisor", actorId: "case08-supervisor",
      idempotencyKey: "case08:confirm:1", evidenceIds: ["PHOTO-CN-AQ-02-038"], data: { validation }, current: returned,
    })).not.toThrow();
    expect(() => validateDomainCommand({
      caseId: "B008", command: "confirm_event", actorRole: "supervisor", actorId: "AQ-FIELD-02",
...
describe("case 09 server-side continuity gate", () => {
  ...
  it("rejects a 352-second gap and accepts a continuous recovery segment", () => {
    const current = projection("B009", "资料已核对", payload, { retrieval });
    const gapRows = [
    ...
    expect(() => validateDomainCommand({
      caseId: "B009", command: "create_inspection_order", actorRole: "supervisor", actorId: "case09-supervisor",
      ...
      current: projection("B009", "资料已核对", payload, {
