...
describe("Case 04 V5 申请材料补正与双岗复核", () => {
  ...
  it("数据质量档案以 applications.csv 为页面权威并隔离 UCI 支持数据", () => {
    const profile = JSON.parse(readFileSync(resolve(
      process.cwd(),
      "../../evidence/data-quality/case-B004-credit-review-profile.json",
    ), "utf8"));

    expect(profile.dataset).toMatchObject({
      path: "dataset/B004-credit-human-review/applications.csv",
    ...
    expect(profile.supportingDataset).toMatchObject({
      path: "dataset/B004-credit-human-review/case.csv",
      rows: 30000,
