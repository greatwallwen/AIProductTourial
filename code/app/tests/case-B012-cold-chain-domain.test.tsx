...
const selected: CaseProjection = { caseId: "B012", objectId: "B012-CCI-2026-001-CN-SC-PZ-01", state: "待调查", version: 0, payload: rows[2], updatedAt: "2026-07-26T07:00:00.000Z" };

function props(overrides: Partial<CaseWorkbenchProps> = {}): CaseWorkbenchProps {
  return {
    definition: getCaseDefinition("B012")!, objects: [selected], selected, events: [], metrics: [], datasetRowCount: rows.length, sceneRows: rows,
...
function event(data: Record<string, unknown>): CaseEvent {
  return { eventId: "evt-12-domain", caseId: "B012", objectId: selected.objectId, command: "open_investigation", actor: { id: "quality_reviewer", role: "quality_reviewer" }, fromState: "待调查", toState: "调查中", version: 1, evidenceIds: [], data, occurredAt: "2026-07-26T07:10:00.000Z" };
}
...
describe("case 12 investigation and route aggregate", () => {
  it("opens a typed investigation aggregate over real route events with stable idempotency", () => {
    ...
    expect(options.idempotencyKey).toBe("case-B012:investigation:CCI-2026-001:route:CN-SC-PZ-01:open_investigation:v0");
  ...
  it("restores task, event, and receipt evidence before supervisor cosign", () => {
    ...
    expect(onCommand.mock.calls[0][2]).toEqual(expect.objectContaining({
      idempotencyKey: "case-B012:investigation:CCI-2026-001:route:CN-SC-PZ-01:quality_cosign:v1",
