...
const objects: CaseProjection[] = rows.map((payload) => ({ caseId: "11", objectId: `11-${payload.evaluation_id}-${payload.candidate_id}`, state: "待会签", version: 0, payload, updatedAt: "2026-07-26T05:00:00.000Z" }));
const selected = objects[1];

function props(overrides: Partial<CaseWorkbenchProps> = {}): CaseWorkbenchProps {
  return {
    definition: getCaseDefinition("11")!, objects, selected, events: [], metrics: [], datasetRowCount: 3, sceneRows: rows,
...
function event(data: Record<string, unknown>): CaseEvent {
  return { eventId: "evt-11-domain", caseId: "11", objectId: selected.objectId, command: "request_release_evidence", actor: { id: "release_manager", role: "release_manager" }, fromState: "待会签", toState: "补测中", version: 1, evidenceIds: [], data, occurredAt: "2026-07-26T06:00:00.000Z" };
}
...
describe("case 11 candidate admission aggregate", () => {
  it("submits one typed candidate aggregate with a stable idempotency key", () => {
    ...
    expect(options.idempotencyKey).toBe("case-11:candidate:MODEL-ADMISSION-001:request_release_evidence:v0");
  ...
  it("merges event, selected.task, and receipt data before chairman approval", () => {
    ...
    expect(onCommand.mock.calls[0][2]).toEqual(expect.objectContaining({
      idempotencyKey: "case-11:candidate:MODEL-ADMISSION-001:approve_canary:v1",
