...
describe("CaseCatalog", () => {
  it("opens with one selected business scene and a user-controlled 20-case filmstrip", () => {
    ...
    expect(within(stage).getByText("发票：01-C496116-M")).toBeVisible();
    ...
    expect(within(filmstrip).getAllByRole("button", { name: /^选择案例 B\d{2}/ })).toHaveLength(20);

    fireEvent.click(within(filmstrip).getByRole("button", { name: /选择案例 B02/ }));
  ...
  it("keeps each case journey specific instead of repeating a shared generic loop", () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /选择案例 B03/ }));
