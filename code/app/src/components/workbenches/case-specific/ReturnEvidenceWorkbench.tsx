...
export function ReturnEvidenceWorkbench(props: CaseWorkbenchProps) {
  ...
  return (
    <main className={styles.console}>
      ...
      <footer className={styles.footer}><span>{props.selected.objectId} · {props.selected.state}</span><button type="button" aria-label={`恢复案例 ${props.definition.id}`} onClick={props.onReset} disabled={props.busy}><RotateCcw size={16} />恢复案例</button></footer>
