...
**规则明确、正确结果可稳定计算。**  
例如统一日期格式或计算报名人数。优先使用普通程序，因为结果更稳定，也更容易检查。

**一次生成已经满足目标。**  
例如根据已提供的完整信息起草一段活动介绍。可使用单次模型调用，并由人决定是否采用。

**步骤多，但顺序固定。**  
例如“读取文件—转换格式—生成摘要—存入指定目录”，若每步和失败分支都能预先写清，固定工作流通常更合适。多步或多次调用不自动构成 AI 循环。

**无法获得可靠反馈。**  
如果系统没有证据判断结果是否接近目标，就没有可靠依据决定继续。更合适的做法是让系统提供候选方案，由人评审，或者先重新定义可检查的目标。

**错误影响重大或不可逆。**  
付款、医疗、法律结论、生产删除等行动应设置清楚的人工授权。系统可以整理材料或生成草稿，但关键决定和执行不能因为模型措辞自信就自动发生。

**成本、时延或资源不允许多轮运行。**  
如果一次处理必须在极短时间完成，或者多轮执行的费用超过任务价值，应选择普通程序、单次调用或固定流程。

**目标含糊或相互冲突。**  
例如既要求“不增加任何成本”，又要求“无条件提高活动质量”。系统不能替责任主体决定价值取舍，应先由人澄清。
...
```mermaid
flowchart LR
    C0["序章<br/>选择起始运行方式"] --> C1["第 1 章<br/>加入最小生成能力"]
    C1 --> C2["后续章节<br/>每章增加一层能力"]
    C2 --> CN["最终<br/>形成受控运行的工程系统"]

    E0["观察实验<br/>隔离一个判断问题"] -.为本章形成直觉.-> C0
    E1["观察实验<br/>暴露局部边界"] -.解释为何需要增量.-> C1
    E2["观察实验<br/>检查新问题"] -.提供设计证据.-> C2

    P0["逐章实操<br/>记录选择与假设"] --> P1["逐章实操<br/>实现最小增量"]
    P1 --> P2["逐章实操<br/>验证、记录局限"]
    P2 --> PN["逐章实操<br/>保留演进证据"]
```
...
选择之后，写下理由和仍需验证的假设。条件不同，答案可以不同；评价重点是论证是否与约束一致。
...
本卡的任务不是决定技术方案，而是列出开始设计前必须澄清的事实，并暂定起始方式。

### 统一判断表

复制并为每张任务卡填写一行：

| 任务 | 目标 | 变化来源 | 步骤可预测性 | 验证证据 | 错误影响与可逆性 | 权限与人工关口 | 成本与时延 | 选择 | 理由 | 待验证假设 |
|---|---|---|---|---|---|---|---|---|---|---|
| 示例：任务卡 1 | 统一有效日期格式 | 异常输入 | 高 | 格式规则与异常清单 | 低，可回滚 | 异常由人处理 | 单次批处理 | 普通程序 | 规则可明确表达 | 输入是否包含无法识别的日期 |

### 参考判断

**任务卡 1：普通程序。**  
规则和正确结果可以明确表达。错误路径是遇到无效日期时不猜测，记录异常并停止处理该条。

**任务卡 2：单次模型调用。**  
任务主要需要一次生成，外部发布由人决定。如果品牌要求或事实材料不完整，应停止采用草稿并请负责人补充。

**任务卡 3：固定工作流。**  
虽然有多个步骤，也可能包含一次模型生成，但顺序和分支已经固定。任一步失败时按照预定路径停止或交给负责人。

**任务卡 4：可进一步评估 AI 循环。**  
每次补充内容会改变下一步，完成证据和两次提醒上限也比较清楚。但“可以评估”不等于已经适合部署；仍需确认消息权限、错误影响、人工终止入口和实际成本。

**任务卡 5：人工主导的辅助系统。**  
系统可以帮助整理和改写创意，但目标缺少共同标准。应由责任主体先定义评价维度并作出取舍，不能让循环靠反复生成自行宣布完成。

**任务卡 6：人工主导的辅助系统。**  
按卡片条件，不应允许系统自动诊断、开药和提交处方。系统至多在合规范围内辅助整理信息；持证专业人员必须复核并承担关键决定。若缺少授权或证据，停止外部执行。

**任务卡 7：普通程序。**  
这是“看起来复杂但普通程序更合适”的反例。规则多不等于规则不能写清；确定性计算更容易复现和审计。

**任务卡 8：先由人澄清，暂按人工主导处理。**  
目标、权限、证据和责任都未确定，无法负责任地选择 AI 循环。应先形成“实现方式选择记录”，列出待确认事实。案例负责人批准后，才可决定项目从哪种方式起步。
...
这份记录不要求你选中 AI 循环。理由、约束和证据一致，才是本实验的目标。
