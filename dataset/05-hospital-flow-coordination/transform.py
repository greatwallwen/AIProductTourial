...
def main() -> None:
    ...
    write_json("schema.json", {"schema_version": "2.0", "case_id": "05", "row_count": len(rows), "columns": [{"name": field, "dtype": "string", "nullable": field == "co_sign_status", "sample": rows[0][field]} for field in FIELDS], "data_nature": "deterministic-synthetic-operational", "source_ids": ["DATA-05"]})
    write_json("source.json", {"schema_version": "2.0", "case_id": "05", "seed": SEED, "captured_at": "2026-07-23T00:00:00+08:00", "sources": [{"id": "DATA-05", "title": "公开医疗运营字段结构参考", "url": "https://physionet.org/content/picdb/1.1.0/", "publisher": "PhysioNet", "version": "1.1.0", "license": "credentialing and DUA required", "state": "metadata-only", "redistribution": "no raw data", "course_use": "仅参考公开字段结构；不读取、校准或再分发原始记录"}], "transform_status": "derived-verified", "derived_output": {"path": "case.csv", "rows": len(rows), "sha256": sha256(ROOT / "case.csv"), "data_nature": "deterministic-synthetic-operational"}})
