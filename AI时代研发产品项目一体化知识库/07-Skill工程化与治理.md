...
### 7.6 dogfood：给本书自己的 24 个 skill 上一道扫描门禁
...
跑一下 `node code/tools/skill_lint.mjs`，你会看到本书 24 个 skill：**0 注入、0 危险指令、六槽齐全**（可发布），外加若干「缺类型元数据」的 LOW 告警。这就是把 §7 的治理，从纸面变成一条真能拦住投毒 Skill 的门禁。
```
