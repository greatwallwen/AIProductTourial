...
> 通用语言是同一律在DDD中的核心落地：一个概念在整个系统中只用一个名字，代码、文档、API、数据库表名必须一致。同一律要求「在推理中保持概念指代不变」，DDD把它翻译成工程铁律——「业务语言→代码语言→API语言」三张皮就是同一律被破坏，概念被偷换。
```
...
> 词链断裂就是同一律被破坏的表现：同一个业务概念在定义层叫 `case_num`、在服务层叫 `id`、在路由层叫 `recordId`——三张皮，概念被偷换了三次。Agent 读到三处不同的名字，无法判断它们是否指同一个东西，于是猜错——这不是 Agent 的错，是你的系统在命名上违背了同一律。
```
