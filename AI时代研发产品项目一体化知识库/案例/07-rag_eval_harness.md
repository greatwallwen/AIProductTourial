...
- 真实业务场景：RAG 回答评测台
- 岗位：AI 产品经理 / 应用研发
- 数据或资料：`dataset/rag/corpus 中文医疗语料 + webMedQA 医疗 Q/A（dogfood）`（60 行，异常 1）
...
- 数据：读取 `dataset/rag/corpus 中文医疗语料 + webMedQA 医疗 Q/A（dogfood）`，只使用其中实际存在的字段（问题、期望命中、实际命中、是否通过）。
...
**追问 1**：换中文 webMedQA 金标后 hit@3 几乎 100%。团队说「检索无敌了，可以上线」。你信吗？

- A. 信，100% 就是强
- B. 不信——webMedQA 问题就是从对应段抽取的，gold 与语料同源、评测太容易，测不出真实区分度
- C. 再补点语料就行

<details>
<summary>点破（先选再展开）</summary>

- 若答错：webMedQA 是单段抽取式问答，问题几乎逐字引用它的来源段落，检索当然一抓一个准。gold 和语料同源，hit@3 虚高——这不是检索强，是评测太松。本案因此收紧到 hit@1。再选？
