...
> **在数字化系统中的位置**：能力智能层 · 洞察环节｜**理论→实操**：把 §2.7 的产品判断力与 §8.3 的领域服务（`rfm()` 无状态分层真算），落成「R/F/M 真算 → 分层 → 识别高价值流失 → 定向挽回」的可运行会员运营动作（数据故事见 dataset/design/case_30.md）。
...
交付物：**RFM 分层运营策略**。必含要素（字段/指标链/异常状态/Skill/决策动作/高影响复核）与合格线由自测器六项核对：`node code/tools/check_my_work.mjs 30 你的方案.md`；红线：不越过「不得输出歧视性或不可解释规则」。
...
3. **动手改一改**：对照页内「真实对照」：真实 1665 客户的 R×F 与合成 800 会员的埋点差在哪？再动手改：合成数据的「高价值流失」群占比定义在 `code/tools/fetch-datasets.mjs`（案 30 生成块的 segs，p:0.12），把它改成 0.10 后依次跑 `node code/tools/fetch-datasets.mjs && node code/tools/build_case_data.mjs`，重启 `bash code/run.sh` 刷新 `#/case/30`，看分层怎么变（设计说明见 dataset/design/case_30.md，它只是文档、改它不生效）。
...
- 答不顺就回读本案演示的原理小节 §2.7、§8.3；写成方案后跑 `node code/tools/check_my_work.mjs 30 你的方案.md`，红项会指明缺什么、回哪章补。
</details>

> **小结**：本案用「航空会员价值运营」演示原理 2.7、8.3，落成可运行、可验收的产品判断。运行 `bash code/run.sh` 后访问 `#/case/30`（真后端实时数据）。
