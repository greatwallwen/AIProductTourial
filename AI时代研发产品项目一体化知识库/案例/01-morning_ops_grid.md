...
- 行业：电商零售
- 真实业务场景：电商早会·退货异常预警（细分：把退货当滞后质量信号，早会即派单）
- 岗位：运营主管
...
**Prompt 1：电商早会·退货异常预警（细分：把退货当滞后质量信号，早会即派单） - 问题定义**

```text
请以产品经理身份，用 AI 编程工具（如 Trae、CodeBuddy 等任一 Agent 工具）完成「电商早会·退货异常预警（细分：把退货当滞后质量信号，早会即派单）」的**产品问题定义**（这一步先把问题想清楚，不写代码）：
- 岗位与场景：运营主管 面向「电商早会·退货异常预警（细分：把退货当滞后质量信号，早会即派单）」，把业务判断转成一份可验证的产品问题定义。
...
**Prompt 2：电商早会·退货异常预警（细分：把退货当滞后质量信号，早会即派单） - 方案验收**（注意：outputs/ 交付物由 build_docs 重建覆盖，建议在新分支/对照目录运行）

```text
请以产品经理身份，用 AI 编程工具（如 Trae、CodeBuddy 等任一 Agent 工具）完成「电商早会·退货异常预警（细分：把退货当滞后质量信号，早会即派单）」的**方案验收**（把上一步的问题定义做成可运行原型，并逐项验收）：
...
![电商早会·退货异常预警（细分：把退货当滞后质量信号，早会即派单） · 信息图](../../outputs/product_case_library/svg/case_01_morning_ops_grid.svg)

![电商早会·退货异常预警（细分：把退货当滞后质量信号，早会即派单） · 可运行大屏原型截图](../../assets/screenshots/premium_case_01_morning_ops_grid_desktop.png)
...
> **小结**：本案用「电商早会·退货异常预警（细分：把退货当滞后质量信号，早会即派单）」演示原理 2.1、2.7，落成可运行、可验收的产品判断。运行 `bash code/run.sh` 后访问 `#/case/01`（真后端实时数据）。
