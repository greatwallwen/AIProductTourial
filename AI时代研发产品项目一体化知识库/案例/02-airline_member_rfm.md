# 实操 02：RFM 客户分层｜航空会员价值运营

### 项目场景故事

> **数据性质：教学合成**（固定种子生成，设计说明见 dataset/design/case_02.md）——分层与效应为教学而设，不代表真实业务分布。
...
> **在数字化系统中的位置**：能力智能层 · 洞察环节｜**理论→实操**：把 §2.7 的产品判断力与 §8.3 的领域服务（`rfm()` 无状态分层真算），落成「R/F/M 真算 → 分层 → 识别高价值流失 → 定向挽回」的可运行会员运营动作（数据故事见 dataset/design/case_02.md）。
...
- 公开参考：RFM 会员价值方法论（Arthur Hughes《Strategic Database Marketing》）；本案数据设计见 dataset/design/case_02.md
...
- 输出：RFM 分层运营策略，保存为 `outputs/product_case_library/case_02_airline_member_rfm_问题定义.md`。
- 边界：结论必须回到数据或公开参考（RFM 会员价值方法论（Arthur Hughes《Strategic Database Marketing》）；本案数据设计见 dataset/design/case_02.md）；不得越过「不得输出歧视性或不可解释规则」。
```
...
- 数据：读取 `dataset/reference_data_analysis/2-air_data.csv`，只使用其中实际存在的字段（会员号、卡等级、最近乘机天数、年飞行次数、年消费、分层、里程余额）。
- 指标链：会员数、年飞行次数均值、年消费总额(元)、高价值分层占比、里程余额均值（当前真实值：会员数=800，年飞行次数均值=14.46，年消费总额(元)=43732680，高价值分层占比=55.1%，里程余额均值=46686.77）。
- 原型（技术契约，遵 rules/ 约束：DRY、单文件<800行、TS 类型、中文注释）：在 `code/web`（Vite+React+TS）路由 `#/case/02`，按 `ui_30_airline_member_rfm`（sales_funnel_screen）与设计 `amber-funnel` 渲染；数据经 `build_case_data.mjs` 预计算，不得复用通用表格占位。
- 使用 Skill：用 privacy-boundary 做验收（结构化 Skill 见 skills/pm_skills.md）。
- 输出：RFM 分层运营策略，保存为 `outputs/product_case_library/case_02_airline_member_rfm_方案验收.md`。
...
- UI 差异：本案例采用 `ui_30_airline_member_rfm` + 设计 `amber-funnel`，不得复用通用表格占位；可运行原型见 `#/case/02`。

### 交付物与验收

交付物：**RFM 分层运营策略**。必含要素（字段/指标链/异常状态/Skill/决策动作/高影响复核）与合格线由自测器六项核对：`node code/tools/check_my_work.mjs 2 你的方案.md`；红线：不越过「不得输出歧视性或不可解释规则」。
...
1. **巩固**：打开 `#/case/02`，找出「高价值流失群」——年消费前列、却最近久未乘机的会员，你会给他们发什么权益？
...
> **小结**：本案用「航空会员价值运营」演示原理 2.7、8.3，落成可运行、可验收的产品判断。运行 `bash code/run.sh` 后访问 `#/case/02`（真后端实时数据）。
