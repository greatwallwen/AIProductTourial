...
- 数据或资料：`code/server/tests + verify + routes（本仓库自身·dogfood）`（36 行，异常 8）
- 公开参考：本仓库 code/server/tests、verify_course_package.mjs、routes/api.ts
- 行业字段：失败用例、失败类别、责任模块、首次出现
- 指标链（真实值）：契约断言数 36，接口契约数 16，校验检查项 198，后端模块数 9
...
- 指标链：测试用例数、契约断言数、校验检查项、接口数（当前真实值：契约断言数=36，接口契约数=16，校验检查项=198，后端模块数=9）。
