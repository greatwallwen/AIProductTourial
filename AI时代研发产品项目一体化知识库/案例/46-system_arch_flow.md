...
- 行业字段：子系统、职责、接口、契约
- 指标链（真实数据）：子系统数 5，接口契约数 10，依赖边数 5，循环依赖 0
...
- 指标链：子系统数、接口契约数、依赖边数、循环依赖（当前真实值：子系统数=5，接口契约数=10，依赖边数=5，循环依赖=0）。
...
2. **你应看到**：指标链（子系统数 / 接口契约数 / 依赖边数 …）、异常队列与责任对象、行动入口，数据全部来自真实后端实时计算。
