...
## 逻辑思维方法论：从总体通过到局部阻断的准入逻辑

本案例的方法论链条如下：

1. **证伪思维**：候选模型做了九项本地评测，八项通过——但准入测试的验收口径不是"证明了模型可靠"，而是"未发现可以排除准入的反例"。唯一的阻断项是地区切片准确率差：实测 4.7%，要求不高于 3.0%，而且对应切片材料缺失。总体成绩再漂亮，也不能填补这份空白——8 项通过不能抵消 1 项阻断，阻断项就是反例，反例的存在直接否决准入。

2. **全集/子集辨析**：9 项评测总体 8 项通过，但地区切片准确率 4.7% 高于 3.0% 门槛——总体通过率是全集指标，地区切片是子集指标，子集不达标时全集的"8/9 通过"不能覆盖局部风险。地区切片对应材料缺失，意味着这个子集不仅指标不达标，连复核材料都不全；平均值再漂亮也填不上这份空白，必须能下钻到单切片单指标看阻断原因。

3. **归纳三前提**：本地 9 项评测 720 个样本，想归纳"模型可以发布"——但本地评测只测了模型能力，没有真实评测时间成本和生产流量作参照，720 个样本是否覆盖足够场景也未知。参照系和数据量都不齐，结论只能停在"本地评测通过"，不能推出"生产发布可靠"。公开 Qwen 元数据只说明候选来源，不能成为准入理由。
