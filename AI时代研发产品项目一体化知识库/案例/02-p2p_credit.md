...
>
> <img src="../../assets/vendor/lucide/built/lightbulb.svg" width="14" alt="" style="vertical-align:-2px" /> **洞见**：信用分层的价值不在算出一个分数，而在把「征信空白、历史零成功」的薄档拎出来做风险处置。本案真算 20349 笔人人贷，整体放款率 20.6%——但反直觉：薄档放款率 52% 远高于优质 6%，无征信(32%) 高于有征信(16%)，授信额度≥5万的仅 1.5% 融到款、<5万 的 40.3%。因为放款成功主要由借款规模驱动（小额易融、大额难融），与信用强弱负相关。放款成功 ≠ 信用好 ≠ 还得起，它是被规模混淆的信号，绝不能当信用度量——这正是本案最该记住的一条。 细分下钻：把借款人按「历史成功次数=1（新客·本真集下限）vs ≥2（复借）」二分（实测该列 min=1/max=32，按数据真实切分，不臆造首贷=0）——同一 CC0 真集现成字段。驱动放款的因子不同：复借有历史行为可依、新客只能靠征信/额度/文案，故差异化风控，别用一套规则套所有人。
>
...
- 行业：P2P信贷
- 真实业务场景：大陆 P2P 信贷·新客 vs 复借 差异化放款与信用画像分层
- 岗位：信贷风控产品经理
...
**Prompt 1：大陆 P2P 信贷·新客 vs 复借 差异化放款与信用画像分层 - 问题定义**

```text
请以产品经理身份，用 AI 编程工具（如 Trae、CodeBuddy 等任一 Agent 工具）完成「大陆 P2P 信贷·新客 vs 复借 差异化放款与信用画像分层」的**产品问题定义**（这一步先把问题想清楚，不写代码）：
- 岗位与场景：信贷风控产品经理 面向「大陆 P2P 信贷·新客 vs 复借 差异化放款与信用画像分层」，把业务判断转成一份可验证的产品问题定义。
...
**Prompt 2：大陆 P2P 信贷·新客 vs 复借 差异化放款与信用画像分层 - 方案验收**（注意：outputs/ 交付物由 build_docs 重建覆盖，建议在新分支/对照目录运行）

```text
请以产品经理身份，用 AI 编程工具（如 Trae、CodeBuddy 等任一 Agent 工具）完成「大陆 P2P 信贷·新客 vs 复借 差异化放款与信用画像分层」的**方案验收**（把上一步的问题定义做成可运行原型，并逐项验收）：
...
![大陆 P2P 信贷·新客 vs 复借 差异化放款与信用画像分层 · 信息图](../../outputs/product_case_library/svg/case_02_p2p_credit.svg)

![案例02 · P2P 信贷 授信额度×历史成功次数 散点（色分放款结果）](../../outputs/product_case_library/svg/fig_credit_scatter.svg)

![大陆 P2P 信贷·新客 vs 复借 差异化放款与信用画像分层 · 可运行大屏原型截图](../../assets/screenshots/premium_case_02_p2p_credit_desktop.png)
...
> **所以真正的一课**：放款成功≠还得起——它被借款规模严重混淆（薄档 52% > 优质 6%、大额几乎融不到款）；「放款率」是转化信号，绝不能当信用/还款能力的度量。

> **小结**：本案用「大陆 P2P 信贷·新客 vs 复借 差异化放款与信用画像分层」演示原理 2.7、8.3，落成可运行、可验收的产品判断。运行 `bash code/run.sh` 后访问 `#/case/02`（真后端实时数据）。
