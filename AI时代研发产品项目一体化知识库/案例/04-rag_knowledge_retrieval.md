...
>
> <img src="../../assets/vendor/lucide/built/lightbulb.svg" width="14" alt="" style="vertical-align:-2px" /> **洞见**：RAG 的价值不在「能检索」，而在「只召回最相关的几段」——本案真做了两阶段：cosine 粗召回 top-10 → 模拟 Cross-Encoder 的词项覆盖重排 top-3，把 194 篇语料压到 3 段喂给模型，降本又聚焦。
>
> <img src="../../assets/vendor/lucide/built/alert-triangle.svg" width="14" alt="" style="vertical-align:-2px" /> **常见坑**：① 只召回不重排，相关度不够；② 把低相似片段当事实回答；③ 语料不切分/不更新，检索质量随时间劣化。
...
- 真实业务场景：产品知识库语义检索
- 岗位：AI 产品经理
- 数据或资料：`skills/external/pm-skills-deanpeters`（194 行，异常 0）
- 公开参考：本地：skills/external/pm-skills-deanpeters（RAG 语料）｜pgvector github.com/pgvector/pgvector
- 行业字段：查询、命中文档、相似度、片段
- 指标链（真实数据）：语料篇数 194，语料总字(万) 191，平均篇幅(字) 9838，覆盖主题数 8
...
- 数据：读取 `skills/external/pm-skills-deanpeters`，只使用其中实际存在的字段（查询、命中文档、相似度、片段）。
...
**Prompt 2：产品知识库语义检索 - 方案验收**（注意：outputs/ 交付物由 build_docs 重建覆盖，建议在新分支/对照目录运行）
...
![产品知识库语义检索 · 信息图](../../assets/course/image2/diagrams/case_04_rag_knowledge_retrieval.png)

![产品知识库语义检索 · 可运行大屏原型截图](../../assets/screenshots/premium_case_04_rag_knowledge_retrieval_desktop.png)
...
本案是「让评测真实调用被测检索系统并驱动一次改进」纵向链的主入口，不再复制九套相同的跟做步骤。完成 [RAG 评测主实验](../../course/labs/04-evals-data.md) 的 活动 09：先运行受控命令，再提交结构化证据与三角色评分。

页面观察入口仍为 `bash code/run.sh` → `#/case/04`；观察不是完成证据，活动契约以 `course/activities.json` 为准。
...
> **小结**：本案用「产品知识库语义检索」演示原理 1.3、3.3，落成可运行、可验收的产品判断。运行 `bash code/run.sh` 后访问 `#/case/04`（真后端实时数据）。
