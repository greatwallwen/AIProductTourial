...
> **本案例演示/验证**：原理 3.0、2.3｜**采用设计** `cyan-matrix`（见 [design/cyan-matrix.md](../../design/cyan-matrix.md)）
...
> **方法论落点**：单个案例 = SDD 流水线（§3.0）上一个可验收的小任务；一个中大型系统 = 许多这样的任务按方法论编排起来。
...
- 行业字段：步骤、工件、状态、产出
- 指标链（真实数据）：宪法条款 18，子系统数 5，门禁检查项 170，架构图数 7
...
- UI 差异：本案例采用 `ui_51_sdd_build` + 设计 `cyan-matrix`，不得复用通用表格占位；可运行原型见 `#/case/08`。
...
- 答不顺就回读本案演示的原理小节 §3.0、§2.3；写成方案后跑 `node code/tools/check_my_work.mjs 8 你的方案.md`，红项会指明缺什么、回哪章补。
</details>

### 被追问（grill-me · 先自己答，再展开）

> 教员式追问：不给你标准答案，先逼你选、再点破误区。页内 `#/case/08` 有可交互版（答错即追问）。

**追问 1**：有了规格文档和阶段门禁，LLM 就能可靠地建大系统了吧？
...
- 若选「能，SDD 是银弹」：SDD 非银弹——LLM 仍非确定、可能瀑布化；它把 LLM 从创意写手变成规格工程师，但必须配「三绿」这类高确定性门禁兜底。
- 若选「能，只要规格写得够细」：规格再细也挡不住 LLM 漂移/幻觉——方法论要和确定性门禁两条腿走路，少一条就上线才炸。
- 答对后再想一层：对。那为赶进度，想砍掉「③澄清」或「⑦门禁」一步，砍哪个更安全？
</details>

**追问 2**：为赶进度想砍掉 SDD 的「③澄清」或「⑦门禁」一步，砍哪个更安全？
...
- 若选「砍澄清」：跳澄清=把意图留给 AI 猜，攒下「意图债务」，后面全走偏。
- 若选「砍门禁」：跳门禁=对不上架构、上线才炸——门禁正是 SDD 兜底那条腿。
- 答对后再想一层：对。方法论 + 门禁，两条腿缺一不可。
</details>

> **所以真正的一课**：SDD 把 LLM 变成规格工程师，但非银弹（非确定、会漂移）——必须配高确定性门禁（三绿）兜底；澄清与门禁都不能砍。

> **小结**：本案用「规格驱动系统建造台」演示原理 3.0、2.3，落成可运行、可验收的产品判断。运行 `bash code/run.sh` 后访问 `#/case/08`（真后端实时数据）。
