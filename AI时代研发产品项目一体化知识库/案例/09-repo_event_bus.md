...
- 真实业务场景：仓库事件总线
- 岗位：平台工程师
- 数据或资料：`git log + 门禁记录（本仓库自身·dogfood 真实事件流）`（92 行，异常 0）
- 公开参考：本仓库 git 历史（真实、不可变、可复算）
- 行业字段：提交哈希、提交时间、提交标题、迭代版本、门禁检查项
- 指标链（真实数据）：事件总数 92，时间跨度天 5，迭代版本数 14，门禁检查项 154
...
- 指标链：事件总数、时间跨度天、迭代版本数、门禁检查项（当前真实值：事件总数=92，时间跨度天=5，迭代版本数=14，门禁检查项=154）。
...
![仓库事件总线 · 信息图](../../assets/course/image2/diagrams/case_09_repo_event_bus.png)
...
本案属于「从规格、任务和契约走到可追溯系统事件」链的支持素材。打开 `#/case/09` 核对专属页面与真实接口；核心动手、评分和完成证据集中在该链的主实验，避免重复做同一套运行/观察/改动/自测。
