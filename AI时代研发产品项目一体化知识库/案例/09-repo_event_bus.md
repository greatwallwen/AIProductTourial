...
>
> <img src="../../assets/vendor/lucide/built/lightbulb.svg" width="14" alt="" style="vertical-align:-2px" /> **洞见**：事件溯源的最小教学标本不需要 Kafka：任何 git 仓库都自带一条不可变事件日志，状态（如「现在门禁有多少检查项」）可以从事件流重放推导。 更对照一个大型国产开源项目：alibaba/nacos 近 600 个真实提交事件、跨约 300 天、含合并 DAG——量级差几档，但事件溯源铁律一样：只读、不可变、可重放，改公共历史=总线失信。细分抓手：拿事件流做「重放到任意历史版本」，而非拿提交数当绩效 KPI（古德哈特）。
>
...
- 行业：软件工程
- 真实业务场景：仓库事件总线·事件溯源（本仓库 dogfood + 大型国产开源项目 nacos 对照）
- 岗位：平台工程师
- 数据或资料：`git log + 门禁记录（本仓库自身·dogfood 真实事件流）`（123 行，异常 2）
- 公开参考：本仓库 git 历史（真实、不可变、可复算）
- 行业字段：提交哈希、提交时间、提交标题、迭代版本、门禁检查项
- 指标链（真实数据）：事件总数 123，时间跨度天 11，迭代版本数 15，门禁检查项 187
...
**Prompt 1：仓库事件总线·事件溯源（本仓库 dogfood + 大型国产开源项目 nacos 对照） - 问题定义**

```text
请以产品经理身份，用 AI 编程工具（如 Trae、CodeBuddy 等任一 Agent 工具）完成「仓库事件总线·事件溯源（本仓库 dogfood + 大型国产开源项目 nacos 对照）」的**产品问题定义**（这一步先把问题想清楚，不写代码）：
- 岗位与场景：平台工程师 面向「仓库事件总线·事件溯源（本仓库 dogfood + 大型国产开源项目 nacos 对照）」，把业务判断转成一份可验证的产品问题定义。
- 数据：读取 `git log + 门禁记录（本仓库自身·dogfood 真实事件流）`，只使用其中实际存在的字段（提交哈希、提交时间、提交标题、迭代版本、门禁检查项）。
- 指标链：事件总数、时间跨度天、迭代版本数、门禁检查项（当前真实值：事件总数=123，时间跨度天=11，迭代版本数=15，门禁检查项=187）。
...
**Prompt 2：仓库事件总线·事件溯源（本仓库 dogfood + 大型国产开源项目 nacos 对照） - 方案验收**（注意：outputs/ 交付物由 build_docs 重建覆盖，建议在新分支/对照目录运行）

```text
请以产品经理身份，用 AI 编程工具（如 Trae、CodeBuddy 等任一 Agent 工具）完成「仓库事件总线·事件溯源（本仓库 dogfood + 大型国产开源项目 nacos 对照）」的**方案验收**（把上一步的问题定义做成可运行原型，并逐项验收）：
...
![仓库事件总线·事件溯源（本仓库 dogfood + 大型国产开源项目 nacos 对照） · 信息图](../../outputs/product_case_library/svg/case_09_repo_event_bus.svg)

![仓库事件总线·事件溯源（本仓库 dogfood + 大型国产开源项目 nacos 对照） · 可运行大屏原型截图](../../assets/screenshots/premium_case_09_repo_event_bus_desktop.png)
...
> **小结**：本案用「仓库事件总线·事件溯源（本仓库 dogfood + 大型国产开源项目 nacos 对照）」演示原理 7.2、9.4、2.3，落成可运行、可验收的产品判断。运行 `bash code/run.sh` 后访问 `#/case/09`（真后端实时数据）。
