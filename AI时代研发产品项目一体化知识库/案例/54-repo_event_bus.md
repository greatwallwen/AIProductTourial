# 实操 54：系统架构｜仓库事件总线（事件驱动·dogfood）
...
- 真实业务场景：仓库事件总线
- 岗位：平台工程师
- 数据或资料：`git log + 门禁记录（本仓库自身·dogfood 真实事件流）`（91 行，异常 0）
- 公开参考：本仓库 git 历史（真实、不可变、可复算）
- 行业字段：提交哈希、提交时间、提交标题、迭代版本、门禁检查项
- 指标链（真实数据）：事件总数 91，时间跨度天 5，迭代版本数 13，门禁检查项 154
...
- 指标链：事件总数、时间跨度天、迭代版本数、门禁检查项（当前真实值：事件总数=91，时间跨度天=5，迭代版本数=13，门禁检查项=154）。
...
- 输出：仓库事件流重放报告（版本聚合时间线 + 门禁红绿标注 + 回退复盘清单），保存为 `outputs/product_case_library/case_54_repo_event_bus_问题定义.md`。
...
- 输出：仓库事件流重放报告（版本聚合时间线 + 门禁红绿标注 + 回退复盘清单），保存为 `outputs/product_case_library/case_54_repo_event_bus_方案验收.md`。
...
交付物：**仓库事件流重放报告（版本聚合时间线 + 门禁红绿标注 + 回退复盘清单）**。必含要素（字段/指标链/异常状态/Skill/决策动作/高影响复核）与合格线由自测器六项核对：`node code/tools/check_my_work.mjs 54 你的方案.md`；红线：不越过「事件流只读；不得据单条提交评价个人绩效」。
...
- 答不顺就回读本案演示的原理小节 §7.2、§9.4、§2.3；写成方案后跑 `node code/tools/check_my_work.mjs 54 你的方案.md`，红项会指明缺什么、回哪章补。
</details>
