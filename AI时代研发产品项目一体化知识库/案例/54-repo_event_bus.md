...
- 明确岗位、指标链、异常状态与决策动作。
- 使用 `harness-builder` 与 `traceability-check` 完成分析，产出 `undefined`，用 `adr-authoring` 验收。
...
- 真实业务场景：仓库事件总线
- 岗位：平台工程师
- 数据或资料：`git log + 门禁记录（本仓库自身·dogfood 真实事件流）`（86 行，异常 0）
- 公开参考：本仓库 git 历史（真实、不可变、可复算）
- 行业字段：提交哈希、提交时间、提交标题、迭代版本、门禁检查项
- 指标链（真实数据）：事件总数 86，时间跨度天 5，迭代版本数 12，门禁检查项 139
...
- 指标链：事件总数、时间跨度天、迭代版本数、门禁检查项（当前真实值：事件总数=86，时间跨度天=5，迭代版本数=12，门禁检查项=139）。
...
- 输出：undefined，保存为 `outputs/product_case_library/case_54_repo_event_bus_问题定义.md`。
...
- 数据：读取 `git log + 门禁记录（本仓库自身·dogfood 真实事件流）`，只使用其中实际存在的字段（提交哈希、提交时间、提交标题、迭代版本、门禁检查项）。
- 指标链：事件总数、时间跨度天、迭代版本数、门禁检查项（当前真实值：事件总数=86，时间跨度天=5，迭代版本数=12，门禁检查项=139）。
- 原型（技术契约，遵 rules/ 约束：DRY、单文件<800行、TS 类型、中文注释）：在 `code/web`（Vite+React+TS）路由 `#/case/54`，按 `ui-eventbus`（workflow_pipeline）与设计 `emerald-flow` 渲染；数据经 `build_case_data.mjs` 预计算，不得复用通用表格占位。
- 使用 Skill：用 adr-authoring 做验收（结构化 Skill 见 skills/pm_skills.md）。
- 输出：undefined，保存为 `outputs/product_case_library/case_54_repo_event_bus_方案验收.md`。
- 验收条件：指标链回到真实数据、异常可追踪、行动入口明确；不得越过「事件流只读；不得据单条提交评价个人绩效」；`node code/tools/verify_course_package.mjs` 必须 ALL GREEN。
```

### 图形/原型/表单

![仓库事件总线 · 信息图](../../outputs/product_case_library/svg/case_54_repo_event_bus.svg)

![仓库事件总线 · 可运行大屏原型截图](../../assets/screenshots/premium_case_54_repo_event_bus_desktop.png)

- 图形类型：repo_event_bus（设计 emerald-flow）
- 看图顺序：先看版本聚合柱（迭代节奏）→ 再看时间线明细 → 最后想：你的系统哪条日志能这样重放？
- UI 差异：本案例采用 `ui-eventbus` + 设计 `emerald-flow`，不得复用通用表格占位；可运行原型见 `#/case/54`。

### 交付物与验收

交付物：**undefined**。必含要素（字段/指标链/异常状态/Skill）与合格线由自测器逐项核对：`node code/tools/check_my_work.mjs 54 你的方案.md`；红线：不越过「事件流只读；不得据单条提交评价个人绩效」。

### 跟着做（动手复现）

1. 起服务：`bash code/run.sh`，浏览器打开 `#/case/54`（本案专属大屏）。
...
4. **自测产出**：`node code/tools/check_my_work.mjs 54 你的方案.md`——红项指明缺什么、回哪章补。
...
> **小结**：本案用「仓库事件总线」演示原理 7.2、9.4、2.3，落成可运行、可验收的产品判断。运行 `bash code/run.sh` 后访问 `#/case/54`（真后端实时数据）。
