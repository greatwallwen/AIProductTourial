...
- 真实业务场景：交付门禁看板
- 岗位：项目/交付负责人
- 数据或资料：`verify 检查项 + 守卫规则 + 角色覆盖（本仓库自身·dogfood）`（198 行，异常 0）
- 公开参考：本仓库 verify_course_package.mjs 检查与守卫、案例与角色覆盖
- 行业字段：门禁项、类别、状态、责任
- 指标链（真实值）：门禁检查项 198，门禁类别 6，案例数 17，覆盖角色数 3
...
- 指标链：门禁检查项、守卫规则数、案例数、覆盖角色数（当前真实值：门禁检查项=198，门禁类别=6，案例数=17，覆盖角色数=3）。
