...
> **一句话**：AI 时代，你不再手动做每件事，而是**设计一个能自动干活、还能自我检查的系统**。它三个零件——**Loop**（让 AI 循环干活的流水线）、**Skills**（把你的判断沉淀成可复用的技能包）、**验证 / evals**（拿一组测试题给 AI 打分，关键处再加人把关）。**本书只做这一个承诺：教你设计会自检的 Loop 系统**——不教提示词小技巧、不教求职、不按角色切三份；研发/产品/项目背景都能读（科普文风），但主线只有一条。全书用 10 个真实案例演示、验证。
>
> **前置**：会用浏览器和命令行即可，无需先懂 AI 或会写代码。
>
> **读完你能**：看懂 AI 产品/系统的底层概念，掌握「设计 Loop → 用 Skills → 靠验证把关」这套操作模型，并能把 10 个真实行业案例跑起来、改起来。
...
| [第二部分 · 10 案例](案例/README.md) | 第一部分 | 入门→高阶 | 每例 ~15min | 所有人，**边读边跑、动手验证** |
...
- 编号说明：案例已于 v21 重编号为 01–10（旧号 30/41/44/45/46/49/51/54 依次对应 02–09）；旧↔新完整对照见 [README-cn](../README-cn.md)。
- [实操 01 · 电商早会·退货异常预警（细分：把退货当滞后质量信号，早会即派单）](案例/01-morning_ops_grid.md)
- [实操 02 · 大陆 P2P 信贷·新客 vs 复借 差异化放款与信用画像分层](案例/02-p2p_credit.md)
- [实操 03 · 零售综合闭环·高价值流失会员挽回（细分：真实 RFM 锁定 M高R大 群）](案例/03-retail_capstone_board.md)
- [实操 04 · 医疗健康知识库语义检索](案例/04-rag_knowledge_retrieval.md)
...
- [实操 10 · 运营商·客户投诉升级分诊与 SLA 违约预警](案例/10-telecom_complaint_escalation.md)
