...
四问之外还有一个判断维度：**这个环节的验收是客观可验证还是主观判断？** 客观可验证——测试是否通过、分数是否达标、输出是否匹配模板——适合交给 Loop。主观判断——感觉对吗？这是我想做的产品吗？用户会喜欢吗？——不适合 Loop，这是老虎机式判断，必须交给人。没有能打分的传感器就是假闭环，Agent 只会原地打转。
```
...
> 验证剧场跟证伪思维是反的：只查"写没写用例""跑没跑通"不查"对不对"——这是在证实，不是在证伪。书里给的解法是**边车传感器**=不用走完整流程就能验证，把证伪拉回 loop 里，让 AI 每一步都带着证伪意识跑。
...
> 三种债（意图债务、理解债务、认知投降）本质上是**逻辑边界守护**的缺失：意图没交代清楚就交给 Agent 猜，是边界没画清；产出太快读不过来，是边界被速度突破；干脆不想了，是边界被完全放弃。守住边界不是保守，是让 Loop 每一步都在可验证的范围内运行。
