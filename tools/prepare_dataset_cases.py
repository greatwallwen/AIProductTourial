...
CASE_DIRS = {
    "B001": "01-retail-return-evidence",
    "B002": "02-member-value-experiment",
    "B003": "03-local-service-voc",
    "B004": "04-credit-human-review",
    "B005": "05-hospital-flow-coordination",
    "B006": "06-beijing-air-quality-audit",
    "B007": "07-instant-retail-architecture",
    "B008": "08-aquaculture-event-response",
    "B009": "09-metro-agentic-rag",
    "B010": "10-telecom-complaint-orchestration",
    "B011": "11-model-release-multi-agent",
    "B012": "12-vaccine-cold-chain",
}
SOURCE_IDS = {
    "B001": ["DATA-01"],
    "B002": ["DATA-02"],
    ...
    "B005": ["DATA-05"],
    "B006": ["DATA-06"],
    "B007": ["DATA-07", "COURSE-OPS-07"],
    "B008": ["DATA-08A", "COURSE-OPS-08"],
    "B009": ["DATA-09", "COURSE-POLICY-09"],
    "B010": ["MIIT-2025-Q2", "DATA-10", "COURSE-OPS-10"],
    "B011": ["DATA-11", "COURSE-EVAL-11"],
    "B012": ["DATA-12", "POLICY-12", "COURSE-OPS-12"],
}
NATURE = {
    "B001": "public-derived",
    "B002": "public-derived-with-sequence-proxy",
    "B003": "public-derived",
    "B004": "public-derived",
    "B005": "calibrated-synthetic",
    "B006": "public-derived",
    "B007": "public-benchmark-derived-plus-deterministic-synthetic-cn-operations",
    "B008": "deterministic-synthetic-cn-operations-linked-to-public-spatial-index",
    "B009": "public-derived-contiguous-slice-plus-versioned-course-policy",
    "B010": "official-aggregate-anchor-plus-deterministic-synthetic-cn-operations",
    "B011": "public-model-metadata-plus-deterministic-enterprise-evaluation",
    "B012": "public-thermal-benchmark-plus-deterministic-synthetic-cn-operations",
}
...
TRANSFORMS = {
    "B001": (transform_01, "一张 ¥81,768.96 的取消发票", ["取消或负数量是异常代理，不是供应商责任结论。", "人民币金额使用课程冻结汇率 1 GBP = ¥9.10 派生；不是实时汇率，也不代表中国经营事实。", "课程子集来自固定原始切片，不代表全部交易分布。"]),
    "B002": (transform_02, "会员价值分层与干预实验平台", ["源文件没有绝对事件时间和金额，不能声称完整 RFM 或真实客户终身价值。", "value_segment 是课程派生的行为参与度分层。"]),
    "B003": (transform_03, "本地生活客户之声研判台", ["评论样本不代表全部用户，星级分层是确定性课程抽样。", "方面标签用于证据检索和离线评测，不能直接决定需求优先级。"]),
    "B004": (transform_04, "消费信贷人工复核与审计平台", ["risk_signal 只用于课程复核路由，不能自动授信或拒贷。", "历史预测样本不能证明现实公平性。"]),
    "B005": (transform_05, "医院患者流程协同与异常处置中心", ["全部患者和事件均为确定性合成。", "不包含诊断、治疗建议或真实个人信息。"]),
    "B006": (transform_06, "北京空气质量数据口径与证据审计台", ["缺失不是 0，相关性不是污染成因。", "case.csv 是固定抽样；完整派生数据保存在 Parquet。"]),
    "B007": (transform_07, "即时零售履约领域建模与架构演进台", ["公开来源描述为 O2O grocery order set，但未证明是生产观察记录。", "它是架构与算法基准，不证明真实履约改善。"]),
    "B008": (transform_08, "智慧水产多模态监测与事件响应中心", ["中国 GeoTIFF 与哥伦比亚传感器序列是独立来源。", "课程不得伪造行级关联或自动设备控制。"]),
    "B009": (transform_09, "轨道交通设备 Agentic RAG 维护平台", ["已知故障窗口来自公开数据说明。", "异常与检索结果不能自动触发维修。"]),
    "B010": (transform_10, "通信服务投诉升级编排台", ["case.csv 是按工信部公开类别比例生成的确定性课程运营数据，不代表真实投诉。", "FCC 公开记录单独保存在 public-reference.csv，消费者陈述未经监管机构核实。"]),
    "B011": (transform_11, "企业大模型准入评审台", ["Qwen 公开元数据只证明来源锚点，课程合成评测不代表官方或企业真实准入结论。", "仓库代码许可不能替代具体模型权重和使用场景的许可核验。"]),
    "B012": (transform_12, "县域冷链温度偏差调查与质量会签台", ["public-benchmark.csv 是尼泊尔公开热性能数据，case.csv 是中国县域确定性合成运营事件，两者不伪造站点映射。", "任何温度筛查或调查结案都不能自动判定产品可用、失效或放行。"]),
}
